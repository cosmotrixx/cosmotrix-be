...
export interface CMEImage {
  filename: string;
  url: string;
  timestamp: Date;
}

export interface CMEVideo {
  id: number;
...
export class CMEService {
  private static readonly BASE_URLS = {
    'ccor1': 'https://services.swpc.noaa.gov/images/animations/ccor1/',
    'lasco-c2': 'https://services.swpc.noaa.gov/images/animations/lasco-c2/',
    'lasco-c3': 'https://services.swpc.noaa.gov/images/animations/lasco-c3/',
  };

  private static readonly IMAGE_PATTERNS = {
    'ccor1': /(\d{8}_\d{4})_ccor1_1024by960\.jpg/,
    'lasco-c2': /(\d{8}_\d{4})_c2_512\.jpg/,
    'lasco-c3': /(\d{8}_\d{4})_c3_512\.jpg/,
  };
   ...
   */
  private static parseTimestamp(filename: string, type: 'ccor1' | 'lasco-c2' | 'lasco-c3'): Date | null {
    const pattern = this.IMAGE_PATTERNS[type];
    const match = filename.match(pattern);
    
    if (match) {
      const timestampStr = match[1]; // YYYYMMDD_HHMM
      ...
      return new Date(year, month, day, hour, minute, 0); // seconds = 0
    }
  ...
  /**
   * Fetch available CME images from NOAA for a specific type
   */
  static async fetchAvailableImages(type: 'ccor1' | 'lasco-c2' | 'lasco-c3'): Promise<CMEImage[]> {
    const baseUrl = this.BASE_URLS[type];
    const pattern = this.IMAGE_PATTERNS[type];
    ...
    try {
      ...
      const images: CMEImage[] = [];
      
      // Try multiple parsing strategies
      const linkPatterns = [
        /<a[^>]+href="([^"]*\.(jpg|jpeg))"[^>]*>/gi,
        /<a[^>]+href='([^']*\.(jpg|jpeg))'[^>]*>/gi,
        /href="([^"]*\d{8}_\d{4}[^"]*\.(jpg|jpeg))"/gi
      ];
      ...
      // Convert links to CMEImage objects
      for (const filename of foundLinks) {
        try {
          const timestamp = this.parseTimestamp(filename, type);
          if (timestamp) {
            images.push({
              filename,
      ...
      if (images.length === 0) {
        ...
        return await this.enumerateRecentImages(type);
      }

      // Sort by timestamp (newest first)
      const sortedImages = images.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime());
      console.log(`Successfully parsed ${sortedImages.length} images for ${type} type`);
      
      return sortedImages;
    } catch (error) {
      console.error(`Error fetching images for ${type}:`, error);
   ...
   */
  static async enumerateRecentImages(type: 'ccor1' | 'lasco-c2' | 'lasco-c3'): Promise<CMEImage[]> {
    const baseUrl = this.BASE_URLS[type];
    const images: CMEImage[] = [];
    const now = new Date();
    
    // Try the last 24 hours worth of images (every 6 minutes for CME data)
    for (let hoursBack = 0; hoursBack < 24; hoursBack++) {
      for (let minutes = 0; minutes < 60; minutes += 6) {
        const targetTime = new Date(now.getTime() - (hoursBack * 60 + minutes) * 60 * 1000);
        const dateStr = targetTime.toISOString().slice(0, 10).replace(/-/g, ''); // YYYYMMDD
        const timeStr = targetTime.toISOString().slice(11, 16).replace(':', ''); // HHMM
        
        let filename: string;
        switch (type) {
          case 'ccor1':
            filename = `${dateStr}_${timeStr}_ccor1_1024by960.jpg`;
            break;
          case 'lasco-c2':
            filename = `${dateStr}_${timeStr}_c2_512.jpg`;
            break;
          case 'lasco-c3':
            filename = `${dateStr}_${timeStr}_c3_512.jpg`;
            break;
        }
    ...
    console.log(`Found ${images.length} images via direct enumeration for ${type}`);
  ...
  private static async createGifOnCloudinaryFromFrames(
    ...
    const sessionId = `cme-${type}-${Date.now()}`;
    const prefixFolder = `cme/${type}/sequences/${sessionId}`;
   ...
   */
  static async uploadToCloudinary(videoBuffer: Buffer, type: 'ccor1' | 'lasco-c2' | 'lasco-c3'): Promise<string> {
    try {
      console.log(`Uploading video to Cloudinary for ${type} type...`);
      
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const publicId = `cme/${type}/${timestamp}`;
  ...
  /**
   * Save CME video record to database
   */
  static async saveToDatabase(
    ...
    try {
      const result = await pool.query(
        `INSERT INTO cme_videos (type, video_url, image_count, date_range_start, date_range_end)
         VALUES ($1, $2, $3, $4, $5)
         ...
         RETURNING id`,
        [type, videoUrl, imageCount, dateRangeStart, dateRangeEnd]
      );
      
      return result.rows[0].id;
    } catch (error) {
      console.error('Error saving to database:', error);
      throw new Error('Failed to save CME video record to database');
    }
  }

  /**
   * Get latest CME video for a type
   */
  static async getLatestVideo(type: 'ccor1' | 'lasco-c2' | 'lasco-c3'): Promise<CMEVideo | null> {
    const pool = getPool();
    
    try {
      const result = await pool.query(
        `SELECT * FROM cme_videos 
         WHERE type = $1 
      ...
      throw new Error('Failed to get latest CME video');
    }
  }

  /**
   * Process CME data for a type (main workflow)
   */
  static async processCMEData(type: 'ccor1' | 'lasco-c2' | 'lasco-c3', maxImages: number = 12): Promise<CMEVideo> {
    try {
      console.log(`Processing CME data for ${type} type...`);
      ...
      const images = await this.fetchAvailableImages(type);
      
      if (images.length === 0) {
        throw new Error(`No images found for ${type} type`);
      }
      ...
      console.log(`Found ${selectedImages.length} images for ${type} type (arranged chronologically)`);
  ...
  const videoUrl = await this.createGifOnCloudinaryFromFrames(imageBuffers, type, 4);
      ...
      console.log(`✓ Successfully processed CME data for ${type}: ${videoUrl}`);
      ...
      console.error(`Error processing CME data for ${type}:`, error);
      throw error;
    }
  }
}